...
const StandardFieldsLibrary: React.FC = () => {
      ...
      <div className="modal-backdrop visible" onClick={closeFieldModal}>
        <div className="modal-container" onClick={e => e.stopPropagation()}>
          <div className="modal-header">
            ...
            <button className="close-btn" onClick={closeFieldModal}>
              <CloseOutlined />
            </button>
          </div>
          <div className="modal-body">
            <form onSubmit={(e) => {
              ...
              {/* 高级选项的内容区域 */}
              <div className="form-section">
                <div className="section-title" onClick={() => setShowAdvancedOptions(!showAdvancedOptions)} style={{ cursor: 'pointer' }}>
                  {showAdvancedOptions ? <UpOutlined /> : <DownOutlined />}
                  <span style={{ marginLeft: '8px' }}>高级选项</span>
                </div>
  ...
  // 修改handleRowClick函数，当点击表格行时更新selectedField
  const handleRowClick = (field: StandardField) => {
                        ...
                        <>
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              if (selectedField) {
                                // 复制字段逻辑...
                                const copiedField = {...selectedField, name: `${selectedField.name} (复制)`, id: ''};
                                handleAddField(currentActiveGroup.id, copiedField);
                              } else {
                                alert("请先选择一个字段");
                              }
                            }}
                  ...
                  <div className="fields-table-container">
                    {currentActiveGroup.fields.length > 0 ? (
                      <table className="fields-table" id="standard-fields-table">
                        ...
                        <tbody>
                          {currentActiveGroup.fields.map((field, index) => (
                            <tr
                              key={field.id}
                            ...
                            >
                              ...
                              <td>{field.primaryKey ? <KeyOutlined style={{color: '#ff2a6d'}} /> : '—'}</td>
                              <td>{field.notNull ? <CheckCircleOutlined style={{color: '#05d9e8'}} /> : '—'}</td>
                              <td>{field.autoIncrement ? <CheckCircleOutlined style={{color: '#05d9e8'}} /> : '—'}</td>
                      ...
                      <div className="no-fields-message">
                        <InfoCircleOutlined style={{ fontSize: '48px', opacity: 0.5, marginBottom: '16px' }} />
                        <p>该分组暂无字段</p>
                        <p style={{ fontSize: '14px', marginBottom: '24px', opacity: 0.7 }}>点击下方按钮添加第一个字段</p>
                        <button
                          className="add-field-btn"
                          onClick={(e) => {
                            e.stopPropagation();
